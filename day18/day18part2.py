...
def A_Star(start: tuple[int,int], goal: tuple[int,int], h: Callable[[tuple[int,int]], int], blockers: set[tuple[int,int]]):
    ...
    while not openSet.empty(): # is not empty
        ...
        for neighbor in neighbours:
            if 0<=neighbor[0]<X and 0<=neighbor[1]<Y and neighbor not in blockers:
...
for i,b in enumerate(blocks):
    blockers.add(b)
    path = A_Star((0,0),(X-1,Y-1), lambda loc: X-loc[0] + Y-loc[1], blockers)
    print(i)
    if not path:
        assert False, f"No path found for {i=} {b=}"
